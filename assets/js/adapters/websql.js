	...
	*/
	...
	var Adapter = {
		...
		_executeSql : function (SQL, params, successCallback, errorCallback) {
			...
			var success = function(tx,result) {
				if(t.debug) {console.log(SQL, params, " - finished");}
	...
	_.extend(AutoAdapter, {
		...
		create : function (model,success,error) {
			...
			this._executeSql("INSERT INTO auto (`nazwa`,`opis`,`aktywny`) VALUES(?,?,?);",[json.nazwa, json.opis, aktywny], success, error);
		},
		
		destroy : function (model, success, error) {
			var json = model.toJSON();
			this._executeSql("DELETE FROM auto WHERE(`id`=?);",[json.id],success, error);
		},
		...
		update : function (model, success, error, options) {
			...
			if (json.aktywny == 1) {
				this._executeSql("UPDATE auto SET aktywny = 0;",[], function (tx, res) {
					tx.executeSql("UPDATE auto SET `nazwa`=?, opis=?, aktywny =? WHERE(`id`=?);",[json.nazwa, json.opis, json.aktywny, json.id], success, error);
				}, error);
			}
			else {
				this._executeSql("UPDATE auto SET `nazwa`=?, opis=?, aktywny =? WHERE(`id`=?);",[json.nazwa, json.opis, json.aktywny, json.id], success, error);
			}
		}
	});
	...
	_.extend(UstawieniaAdapter, {
		...
		create : function (model,success,error) {
			var json = model.toJSON();
			this._executeSql("INSERT INTO ustawienia (`dlugosc`,`pojemnosc`,`waluta`,`lang`) VALUES(?,?,?,?);",[json.dlugosc, json.pojemnosc, json.waluta, json.lang], success, error);
		},
		...
		update : function (model, success, error, options) {
			var json = model.toJSON();
			this._executeSql("UPDATE ustawienia SET `dlugosc`=?, pojemnosc=?, waluta =?, lang=? WHERE(`id`=?);",[json.dlugosc, json.pojemnosc, json.waluta, json.lang, json.id],
							function(tx,res) {
								if (res.rowsAffected <= 0) {
									// jezeli nie udalo sie zaktualizowac to dodoac jako insert
									tx.executeSql("INSERT INTO ustawienia (`dlugosc`,`pojemnosc`,`waluta`,`lang`,`id`) VALUES(?,?,?,?,?);",[json.dlugosc, json.pojemnosc, json.waluta, json.lang, json.id], success, error);
								}
								else {
									success(tx, res);
								}
